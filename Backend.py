...
class HeartGuardGUI:
    ...
    def ui(self):
        ...
        tk.Label(title_frame, text="❤️ 心盾 · 实时心脏健康监测",
                 font=("微软雅黑", 22, "bold"), bg="#f7f7f7", fg="#d92c2c").pack()

        topf = tk.Frame(self.root, bg="#f7f7f7")
        topf.pack(fill="x", padx=30, pady=5)
        tk.Label(topf, text="设备状态：", font=("微软雅黑", 11), bg="#f7f7f7").pack(side="left")
        tk.Label(topf, textvariable=self.device, font=("微软雅黑", 11, "bold"),
                 fg="#0066cc", bg="#f7f7f7").pack(side="left")
        tk.Label(topf, text=" | 数据显示：全部历史数据",
                 font=("微软雅黑", 9), bg="#f7f7f7", fg="#888").pack(side="left", padx=10)
        ...
        tk.Label(hr_card, text="💓 实时心率", font=("微软雅黑", 14, "bold"), bg="white", fg="#333").pack(pady=(8, 5))
        tk.Label(hr_card, textvariable=self.hr, font=("Arial", 46, "bold"),
                 fg="#e63946", bg="white").pack()
        tk.Label(hr_card, text="单位：次/分钟 (bpm)", font=("微软雅黑", 10), bg="white", fg="#888").pack(pady=(0, 8))

        health_card = tk.Frame(left_col, bg="white", bd=2, relief="ridge")
        health_card.pack(fill="x", pady=5)
        tk.Label(health_card, text="⭐ 心脏健康评分", font=("微软雅黑", 14, "bold"), bg="white", fg="#333").pack(
            pady=(8, 5))
        tk.Label(health_card, textvariable=self.health, font=("Arial", 38, "bold"),
                 fg="#2e8b57", bg="white").pack()
        tk.Label(health_card, textvariable=self.health_lvl, font=("微软雅黑", 12, "bold"),
                 bg="white", fg="#2e8b57").pack(pady=(0, 5))
        tk.Label(health_card, text="单位：分 (0-100)", font=("微软雅黑", 9), bg="white", fg="#888").pack(pady=(0, 8))
        ...
        tk.Label(risk_card, text="⚠️ 风险状态", font=("微软雅黑", 14, "bold"), bg="white", fg="#333").pack(
            pady=(8, 8))
        tk.Label(risk_card, textvariable=self.risk_lbl, font=("微软雅黑", 16, "bold"),
                 bg="white").pack(pady=8, expand=True)
        tk.Label(risk_card, text="综合风险评估", font=("微软雅黑", 9), bg="white", fg="#888").pack(pady=(0, 8))
        ...
        tk.Label(stats_card, text="📊 全部数据统计", font=("微软雅黑", 11, "bold"),
                 bg="white", fg="#333").pack(side="left", padx=15, pady=6)
        stats_inner = tk.Frame(stats_card, bg="white")
        stats_inner.pack(side="right", padx=15, pady=6)
        tk.Label(stats_inner, text="平均心率:", font=("微软雅黑", 9), bg="white").pack(side="left", padx=4)
        tk.Label(stats_inner, textvariable=self.avg_hr, font=("微软雅黑", 9, "bold"),
                 bg="white", fg="#e63946").pack(side="left", padx=2)
        tk.Label(stats_inner, text="bpm", font=("微软雅黑", 8), bg="white").pack(side="left")
        tk.Label(stats_inner, text=" | 最高:", font=("微软雅黑", 9), bg="white").pack(side="left", padx=4)
        tk.Label(stats_inner, textvariable=self.max_hr, font=("微软雅黑", 9, "bold"),
                 bg="white", fg="#ff6600").pack(side="left", padx=2)
        tk.Label(stats_inner, text="bpm", font=("微软雅黑", 8), bg="white").pack(side="left")
        tk.Label(stats_inner, text=" | 最低:", font=("微软雅黑", 9), bg="white").pack(side="left", padx=4)
        tk.Label(stats_inner, textvariable=self.min_hr, font=("微软雅黑", 9, "bold"),
                 bg="white", fg="#3399ff").pack(side="left", padx=2)
        tk.Label(stats_inner, text="bpm", font=("微软雅黑", 8), bg="white").pack(side="left")

        formula_frame = tk.Frame(self.root, bg="#f0f0f0", bd=1, relief="solid")
        formula_frame.pack(fill="x", padx=30, pady=6)
        tk.Label(formula_frame, text="📐 评分计算公式", font=("微软雅黑", 11, "bold"),
                 bg="#f0f0f0", fg="#0055cc").pack(pady=(6, 2))
        tk.Label(formula_frame, textvariable=self.formula, font=("Consolas", 10),
                 bg="#f0f0f0", fg="#333", wraplength=900).pack(pady=(0, 6))

        diag_frame = tk.Frame(self.root, bg="#fff9e6", bd=1, relief="solid")
        diag_frame.pack(fill="x", padx=30, pady=6)
        tk.Label(diag_frame, text="📊 异常指标诊断", font=("微软雅黑", 11, "bold"),
                 bg="#fff9e6", fg="#c92c2c").pack(pady=(6, 3))
        tk.Label(diag_frame, textvariable=self.issue, font=("微软雅黑", 10),
                 bg="#fff9e6", fg="#c92c2c", wraplength=900).pack()
        tk.Label(diag_frame, textvariable=self.range_tip, font=("微软雅黑", 9),
                 bg="#fff9e6", fg="#666", wraplength=900).pack(pady=(3, 6))

        suggest_frame = tk.Frame(self.root, bg="#e8f4f8", bd=1, relief="solid")
        suggest_frame.pack(fill="x", padx=30, pady=6)
        tk.Label(suggest_frame, text="💡 健康建议", font=("微软雅黑", 11, "bold"),
                 bg="#e8f4f8", fg="#0066cc").pack(pady=(6, 3))
        tk.Label(suggest_frame, textvariable=self.suggest, font=("微软雅黑", 10),
                 bg="#e8f4f8", fg="#333", wraplength=900).pack(pady=(0, 6))

        # ====================== 【缩小图表尺寸 + 更紧凑比例】 ======================
        chart_frame = tk.Frame(self.root, bg="white", bd=1, relief="solid")
        chart_frame.pack(fill="both", expand=True, padx=30, pady=8)

        chart_title = tk.Frame(chart_frame, bg="white")
        chart_title.pack(fill="x", pady=(5, 0))
        tk.Label(chart_title, text="📈 心率历史趋势（紧凑视图）", font=("微软雅黑", 11, "bold"),
                 bg="white", fg="#333").pack(side="left", padx=8)

        # 缩小图表：宽度更小 + 高度更低
        self.fig, self.ax = plt.subplots(figsize=(9, 2.8), dpi=90, facecolor='white')
        self.ax.set_ylim(30, 180)
        self.ax.set_xlabel("时间 (秒)", fontsize=9)
        self.ax.set_ylabel("心率 (bpm)", fontsize=9)
        self.ax.grid(True, linestyle='--', alpha=0.5)
        self.ax.axhspan(60, 100, alpha=0.2, color='green', label='健康范围')
        self.ax.yaxis.set_major_locator(MultipleLocator(20))
        self.line, = self.ax.plot([], [], lw=1.2, color="#e63946")
        ...
        tk.Label(status_bar, text="❤️ 心盾健康监护系统 | 永久保存全部心率数据",
                 font=("微软雅黑", 8), bg="#e0e0e0", fg="#666").pack(side="left", padx=10)
